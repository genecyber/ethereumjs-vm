...
module.exports = {
  ...
  SLOAD: function (runState, cb) {
    ...
    stateManager.getContractStorage(contractAddress, key, function (err, value) {
      if (err) return cb(err)
  ...
  SSTORE: function (runState, cb) {
    ...
    stateManager.getContractStorage(contractAddress, key, function (err, found) {
      if (err) return cb(err)
      ...
      stateManager.putContractStorage(contractAddress, contract, key, value, cb)
    })
  },
  ...
  CALL: function (runState, done) {
    ...
    stateManager.getAccount(toAddress, function (err, account, raw) {
      if (!raw) {
  ...
  CALLCODE: function (runState, done) {
    ...
    stateManager.getAccount(toAddress, function (err, account) {
      ...
      if (account.isPrecompiled(toAddress)) {
        ...
        stateManager.getContractCode(account, function (err, code, compiled) {
          if (err) return done(err)
          options.code = code
  ...
  SUICIDE: function (runState, cb) {
    ...
    var contract = runState.contract
    var contractAddress = runState.address
    suicideToAddress = utils.pad(runState.stack.pop(), 20)
    ...
    stateManager.getAccount(suicideToAddress, function(err, toAccount){
      ...
      async.series([
        stateManager.putAccountBalance.bind(stateManager, suicideToAddress, toAccount, newBalance),
        stateManager.putAccountBalance.bind(stateManager, contractAddress, contract, 0),
      ], cb)
    })
  },
